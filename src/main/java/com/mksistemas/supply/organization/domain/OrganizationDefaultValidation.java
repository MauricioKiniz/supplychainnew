...
public interface OrganizationDefaultValidation {
	...
	default void verifyDuplicatedName(OrganizationRepository repository,
			Organization organization) {
		if (repository.existsNameDuplicated(organization.getName()).isPresent())
			throw new BusinessException(ORGANIZATION_NAME_DUPLICATED,
	...
	default void verifyCountryIsoCode(Organization organization) {
		...
		if (!countries.contains(organization.getCountryIsoCode()))
			throw new BusinessException(ORGANIZATION_COUNTRYISOCODE_NOTFOUND,
